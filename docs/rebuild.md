...
3. Restore the runtime repository at the recorded `root` path, using the public repository or `git clone /path/to/kit/runtime.git.bundle /home/astigmatism/apps/local-ai-runtime`. Set `origin` to `https://github.com/astigmatism/local-ai-runtime.git`. On this **new checkout**, select the recorded deployed commit on `main` and set its upstream to `origin/main`. Keep the exact deployed source until recovery succeeds; upgrade afterward.
