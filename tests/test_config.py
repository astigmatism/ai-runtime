...
class ConfigurationTests(unittest.TestCase):
    def test_import_matches_both_independently_recorded_production_profiles(self):
    ...
    def test_context_edit_updates_both_launch_flags_manifest_and_catalog(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = config / 'profiles/daytime-swift.json'
            definition = read(path); definition['context_tokens'] = 98304
            path.write_text(json.dumps(definition))
            result = render(config, BASELINE['host'], 'daytime-swift')
            ...
            self.assertEqual(result['catalog']['display_name'], 'Daytime-Swift (96K)')
    ...
    def test_no_qualification_is_synthesized(self):
        result = render(ROOT / 'config', BASELINE['host'], 'daytime-swift')
        self.assertNotIn('qualified', json.dumps(result))
        self.assertEqual(len(result['artifacts']), 4)
        self.assertEqual(result['catalog']['aliases'], ['local-active', 'daytime-swift'])
